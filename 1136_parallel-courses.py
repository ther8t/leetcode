class Solution:
    def minimumSemesters(self, n: int, relations) -> int:
        prerequisites = {i: set() for i in range(1, n + 1)}
        course = {i: set() for i in range(1, n + 1)}
        for p, c in relations:
            prerequisites[c].add(p)
            course[p].add(c)

        queue = []
        for c in prerequisites:
            if len(prerequisites[c]) == 0:
                queue.append(c)

        count = 0
        queue2 = []
        semester = 0
        while queue2 or queue:
            popped = queue.pop()
            count += 1

            for c in course[popped]:
                prerequisites[c].remove(popped)
                if len(prerequisites[c]) == 0:
                    queue2.append(c)
                    del prerequisites[c]
            if not queue:
