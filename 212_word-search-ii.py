...
class Solution:
    def findWords(self, board, words):
        ...
        def backtrack(r, c, parent, word):
            current_node = parent[board[r][c]]
            if IS_WORD in current_node:
                out.add(word)
                current_node.pop(IS_WORD)

            visited.add((r, c))
            for nr, nc in [(r - 1, c), (r, c - 1), (r + 1, c), (r, c + 1)]:
                if 0 <= nr < rows and 0 <= nc < cols and board[nr][nc] in current_node and (nr, nc) not in visited:
                    backtrack(nr, nc, current_node, word + board[nr][nc])
            visited.remove((r, c))

            if not current_node:
                parent.pop(board[r][c])
    ...
    # # TLE 64/65 This is a good method to learn to distinguish DFS vs BFS vs Backtracking.
    # def findWords(self, board, words):
    #     rows, cols = len(board), len(board[0])
    ...
    #     out = []
    #     for word in words:
    #         if not word or not is_possible(word):
    #             continue
    #         for r, c in char_map[word[0]]:
    #             visited = set()
    #             visited.add((r, c))
